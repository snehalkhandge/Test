...
namespace Myware.Web.API.PreSalesUnit
{
    ...
    */
    public class LocationsController : ODataController
    {
        ...
        public IHttpActionResult Put([FromODataUri] int key, Location location)
        {
        ...
        // GET: odata/Locations(5)/Localities
        [Queryable]
        public IQueryable<Locality> GetLocalities([FromODataUri] int key)
        {
            return db.Locations.Where(m => m.Id == key).SelectMany(m => m.Localities);
        }

        // GET: odata/Locations(5)/User
        [Queryable]
        public SingleResult<User> GetUser([FromODataUri] int key)
        {
            return SingleResult.Create(db.Locations.Where(m => m.Id == key).Select(m => m.User));
        }
